# UGCast — Free Open-Source Alternative to Arcads & MakeUGC
...
- **AI Actors (talking avatars)** — Pick a reusable actor, paste or AI-write a script, choose a voice + emotion, and generate a **lip-synced talking-head UGC ad** (image + TTS → talking-avatar model). Full Arcads-style core loop.
- **Actor library** — Seed a roster of AI-generated stock actors (`npm run seed:actors`), generate brand-new faces from a persona description, or save your own face/product as a reusable actor. Managed at `/actors`.
- **AI scriptwriting, hooks & translation (GPT-5)** — "Write with AI" turns a product + angle into a UGC script; the hook generator drafts multiple opening lines for testing; one-click translate into 30+ languages.
- **Batch / variant testing** — Attach one script to multiple actors, or multiple hooks to one actor, and generate the whole variant set in a single run. Grouped as "variant sets" in the dashboard.
- **Multi-scene ad editor (Remotion)** — Stitch your generated clips into one ad at `/editor`: reorder scenes, burn word-timed captions (derived from each clip's script — no transcription needed), add fade transitions, and lay an AI-generated background music track (Suno). Live preview via `@remotion/player`; final MP4 rendered server-side via `@remotion/renderer` and uploaded back to MUAPI. **Rendering is heavy/long-running** — run it on a normal Node host (`npm start`) or a dedicated render worker, not a short-timeout serverless function (`@remotion/renderer` + `@remotion/bundler` are kept out of the app bundle and loaded dynamically by `/api/render`).
- **Public API** — Create API keys at `/developers` and drive generation programmatically via `POST /api/v1/generate` and `GET /api/v1/actors` (`x-api-key` header).
- **Ad/platform templates** — One-click TikTok / Reels / Shorts / Square Feed / YouTube presets that set the right aspect ratio, duration, and resolution.
...
UGC ads frequently need multiple references — actor face + product + setting. UGCast supports up to **7 reference images per generation**:
...
# OpenAI — powers AI script writing, hook generation, and 30+ language translation
OPENAI_API_KEY="sk-..."
...
UGCast communicates with [Muapi.ai](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=open-ai-ugc) using a webhook pattern:
...
**UGCast** is a community-driven, open-source alternative that gives you the same Arcads/MakeUGC workflow without the closed ecosystem and per-seat pricing.

| | Arcads / MakeUGC | **UGCast** |
| :--- | :--- | :--- |
...
*Looking for a free, open-source alternative to Arcads or MakeUGC? UGCast is a self-hostable AI UGC video ad studio you can customize, rebrand, and resell. No content filters, no vendor lock-in, no per-seat pricing.*
